...
4. In the Bash console let's first run the command `jasmine` and then the command `npm test`. Both tests should pass and you should see green dots and lines printed in the console that look like this...
...
We looked at the 3 step Arrange-Act-Assert pattern. It's powerful because it is simple. Arrange-Act-Assert is a great pattern to follow for writing good functional tests.
