...
You should recognize when you have a repository open on Git by seeing `git: (master)` following the name of your file. This not only tells you that you have an open repository but also that you are working on the master branch.
...
The idea for creating a fork of a repository is similar to creating a branch. In each case you are creating an exact copy of the master repository. The difference lies in the intended use for the copy. When you fork a repo, the intention is to have a starting point for your own project. The updates made to the project are then done on your own computer and are independent of the master.
