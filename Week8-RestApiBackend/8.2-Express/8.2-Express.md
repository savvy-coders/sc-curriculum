...
2.  In `users.js`, let's `require` the `Router` from `express`, construct a new one in the `users` file, then refactor all of the `users` routing from our `http` server.
  ...
  router
    ...
    .post((request, response) => {
      const user = db.get("users").insert(request.body).write();
      response.json(user);
    });
...
app.use(bodyParser.json()).use(morgan('dev')).use('/users', users)
```

4. Now we can repeat the exact same process for pizzas! See if you can create and include an `pizzas` router that exposes the exact same routes as `/users`

5. If done correctly, you should now be able to `curl -X POST -H 'Content-Type: application/json'` data from `users` in our db. Try creating a few posts with the schema outlined above in JSON format!
